// Cookie Guardian — Privacy Content Script
...
(function () {
  ...
  function randNoise() { return Math.floor((Math.random() - 0.5) * 3); }
  ...
  const TRACKING_PARAMS = new Set([
    ...
    'gclid','gclsrc','dclid','gad_source','_gl',
    // Facebook / Meta
    ...
    'ScCid',
    // Generic
    'ref','source','affiliate','partner','campaign','cid','sid',
    'zanpid','tag','igshid','s_kwcid','ef_id',
  ]);
  ...
  const STORAGE_TRACKER_PATTERNS = [
    /^_ga$/i, /^_gid$/i, /^_fbp$/i, /^_fbc$/i,
    /^amplitude_/i, /^mp_/i, /^mixpanel_/i,
    /^ajs_/i, /^analytics_/i,
    /^__hstc$/i, /^hubspot/i,
    /^intercom\./i,
    /^hs_/i,
  ...
  const STORAGE_FUNCTIONAL_PATTERNS = [
    /cart/i, /basket/i, /auth/i, /token/i, /session/i,
    /user/i, /account/i, /login/i, /csrf/i, /xsrf/i,
    /lang/i, /locale/i, /theme/i, /pref/i, /setting/i,
    /wishlist/i, /favorite/i, /draft/i, /remember/i,
  ];
  ...
  function cleanWebStorage(cleanLocal, cleanSession) {
    ...
    function processStorage(storage) {
      ...
      toDelete.forEach(k => { storage.removeItem(k); cleared++; });
    }

    try { if (cleanLocal) processStorage(localStorage); } catch (_) {}
    try { if (cleanSession) processStorage(sessionStorage); } catch (_) {}

    if (cleared > 0) {
      log(`Cleared ${cleared} tracking storage items`);
      try { chrome.runtime.sendMessage({ action: 'storageCleared', count: cleared }); } catch (_) {}
    }
  }


  // ─── Helpers ──────────────────────────────────────────────────────────────
  function log(...args) { console.debug('[Cookie Guardian]', ...args); }
